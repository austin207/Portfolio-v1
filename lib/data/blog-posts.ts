...
Transformer ArchitectureAn easier representation of transformer
...
**Source codes:**[Transformer-Virtue-v2](https://github.com/austin207/Transformer-Virtue-v2.git)
    `,
    author: "Antony Austin",
    publishedAt: "2024-05-15",
    category: "Machine Learning",
    ...
    featured: false,
    image: "/placeholder.svg?height=400&width=800",
    slug: "building-transformer-model-from-scratch-2",
  },
  {
    id: "8",
    title: "The Ultimate n8n Automation Guide",
    excerpt: "Welcome! This guide is designed to take you from the fundamentals of automation to building complex AI-powered agents using n8n. We'll cover the core concepts, tools, practical examples, and advanced techniques.",
    content: `
...
1.**Trigger Node:** An "On Form Submission" trigger.  
   * Imagine a web page where users can book a room by filling out a form with their name and desired room type (e.g., Austin, Deluxe Room).  
...
2.**Action Node:** The data is sent to Airtable.  
   ...
   * The data is automatically added as a new row in an Airtable sheet (e.g., "Austin", "Deluxe Room").
...
3. **Tools:** The LLM has access to tools like:  
   ...
   * \`Gmail_Send\`: Can be told to "Write an email to Austin, invite him to my birthday party," and it will compose and send the email.  
...
8.**Vector Databases, Embedding Models & RAG (Retrieval-Augmented Generation):**     
   ...
   * **RAG:** A technique allowing LLMs to access external knowledge by:  
     - Embedding document chunks and storing them in a vector database.  
     - Embedding the user's query.  
     - Retrieving the most relevant document chunks from the database.  
     - Providing these chunks (as context) to the LLM along with the user's query to generate an informed answer.  
...
3.**Install n8n Globally via npm:**  
   ...
   * Run the following command: npm install n8n \-g  
...
4.**Run n8n:**    
   * Once the installation is complete, you can start n8n by simply typing the following command in your command prompt/terminal:  n8n      
...
4.**Restart n8n:**   
   * Once the update is complete, type the following command in the same or a new command prompt/terminal: n8n     
...
4.**Test the Form Submission Trigger:**  
   ...
   * Fill it out:  
     ...
     * Room: \`Single Room\`  
   * Click **"Submit."**  
   * Go back to n8n. The node should show that it executed successfully and display the data received (e.g., \`your_name: "Austin"\`,\`room_choice: "Single Room"\`). You can view this data in Table, JSON, or Schema format on the right panel.
...
8.**Test the Airtable Node:**    
   ...
   * n8n will use the data from the last successful execution of the "On Form Submission" node (e.g., Austin, Single Room) and attempt to send it to Airtable.  
   * If successful, check your Airtable base. You should see a new row with "Austin" and "Single Room."
...
4.**Add OpenAI Node (LLM for Summarization):**  
  ...
  - You are responsible for customer orders. Your task is to collect incoming information about new orders and create a clear summary that will be sent via email to the team. The email should be signed with "Customer Success Team".
    ...
    - Please provide the following output parameters in JSON format: "email_subject" and "email_content".  
...
2.**Add Trigger Node: On Form Submission:**   
   ...
   * Required: ON  
   * **Test the form:** Click "Test step," fill in a sample name and review (e.g., \`Austin\`, \`I think this was a really good hotel room. I like that it is clean and so on.\`), and submit. Verify data in n8n.
...
- System Prompt example:  
  \`"You are an expert in sentiment analysis... Respond with only one word for sentiment. Also, repeat the user's name if provided. Output as JSON: {"user_name": "NAME", "sentiment": "SENTIMENT"}"\`
- User message to AI would include:  
  \`User Name: {{ $('On Form Submission').item.json.user_name }} Review: {{ $('On Form Submission').item.json.review_text }}\`
...
- **Subject (Expression):**  
  \`Review from {{ $('OpenAI').item.json.user_name }}\`
- **Message (Expression):**  
  \`Sentiment: {{ $('OpenAI').item.json.sentiment }}\`
...
* **Anthropic Claude API:**  
  * Models like \`claude-3-5-sonnet-20240620\` are powerful, especially for long context and strong reasoning/writing.  
...
7. **Testing the RAG Chatbot:**  
   ...
   * **Test Memory:**  
     * "Hey, my name is Austin." (Agent should respond).  
     * "What is my name?" (Agent should remember "Austin").  
   * **Test RAG:**  
...
1. **Prepare Google Doc:**  
   ...
   * Create a Google Doc (e.g., named \`Austin Test Contacts\`).  
   * List contacts, each on a new line or in a simple format, e.g.:  
       
     Austin Test: Austin.test@example.com  
...
2. **Testing the Full System:**  
   ...
   * **User:** "Send an email to Austin Test. Tell him that I like his course."  
   * **Expected Agent Behavior:**  
     1. Main Agent uses \`vector_store_mails\` tool to find "[Austin.test@example.com](mailto:Austin.test@example.com)".  
     ...
     5. Main Agent confirms to user: "I've sent the email to Austin Test letting him know..."  
...
3. **Configure the AI Agent:**  
   ...
   * **System Message (Expression):** This is key to guide the LLM.  
     ...
     4\. Sign off every email with "Austin". (Or your preferred sign-off)  
...
4. **Testing the Quick Email Agent:**  
   ...
   * **Example Prompts:**  
     * "Send an email to \`friend@example.com\`. Subject: Meeting Confirmation. Body: Hi Friend, Just confirming our meeting tomorrow at 8 PM at the local bank. Best, Austin."  
...
**Workflow Goal:** For a YouTube channel owner ("AI with Austin" / "AI Midjourney"), automatically handle sponsorship inquiry emails:
...
2. **Add Gmail Trigger Node ("On Message Received"):**  
   ...
   * **Test:** Click **"Fetch Test Event."** To test effectively, send yourself a sample sponsorship email and a sample non-sponsorship email to the monitored inbox. Fetch one of these (e.g., the sponsorship email).  
     * *Example Sponsorship Email (sent to yourself for testing):* Subject: Sponsorship for your YouTube video Body: Hey Austin, I'd like to offer a sponsorship for your next YouTube video. Our product is an AI tool that generates pictures. I offer €600 for a 60-second integration. \- Peter from PixelAI.
   ...
   * Add an **"OpenAI"** node (or another AI Agent node if preferred for consistency).  
     ...
     * **Messages:**  
       * **Item 1: System Message (Expression):**  
         ...
         You work for a YouTube channel called "AI Midjourney" (or "AI with Austin").  
...
4. **Define Tools and Their Usage:**  
   ...
   * For each tool, specify:  
     ...
     * Example: \`Send_Mail_Tool → Use this tool to send emails. Always sign emails with "Austin".\`  
...
6. **Testing:**  
   ...
   * Example conversation:  
     ...
     * You: "My name is Austin"  
     * Bot: "Nice to meet you, Austin. How can I assist you today?"  
     * You: "What is my name?"  
     * Bot: "Your name is Austin. How can I help you today?"
...
6. **Main AI Agent Node:**  
   ...
   * **System Prompt (Expression \- this will be extensive):**  
     ...
     \*   \*\*\`Gmail\_Tool\`\*\*: Use this for all email-related tasks: reading summaries, sending new emails, replying.  
         ...
         \*   When sending, sign emails with "Austin".  
...
9. **Testing:**  
   ...
   * Example test sequence from course:  
     ...
     * "Send a mail to Austin that I love his course about AI automation." (Email \+ Salty comment)  
...
You now possess a powerful skill set to create impactful automations and intelligent AI agents. Use it wisely, ethically, and creatively. Thank you for investing your time in this course, and I wish you the very best in your automation endeavors. Whether you continue to refine the projects from this course or embark on new ones, the foundation you've built here will serve you well.      
    ...
    publishedAt: "2024-05-15",
    category: "Machine Learning",
    tags: ["n8n", "Automations", "AI workflows", "NLP", "AI", "Agentic AI"],
    readingTime: 15,
    featured: false,
    image: "/placeholder.svg?height=400&width=800",
    slug: "The-Ultimate-n8n-Automation-Guide",
  },
  /*{
    id: "7",
    title: "Building a Transformer Model from Scratch",
